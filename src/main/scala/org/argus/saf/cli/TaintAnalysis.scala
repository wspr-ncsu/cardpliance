...
//  private final val TITLE = "TaintAnalysis"
  def apply(module: TaintAnalysisModules.Value, debug: Boolean, sourcePath: String, outputPath: String, forceDelete: Boolean, guessPackage: Boolean, approach: TaintAnalysisApproach.Value, uirefCC: String, uirefCVC: String, displayWidget: String): Unit = {
    ...
    taintAnalyze(module, apkFileUris.toSet, outputPath, debug, forceDelete, guessPackage, approach, uirefCC,uirefCVC,displayWidget,sourcePath)
  }
  
  def taintAnalyze(module: TaintAnalysisModules.Value, apkFileUris: ISet[FileResourceUri], outputPath: String, debug: Boolean, forceDelete: Boolean, guessPackage: Boolean, approach: TaintAnalysisApproach.Value, uirefCC: String, uirefCVC: String, displayWidget: String,source: String): Unit = {
    ...
    try{
      ...
      apkFileUris.foreach{ fileUri =>
        ...
        try{
          ...
          TaintAnalysisTask(module, Set((fileUri, outputUri)), forceDelete, reporter, guessPackage, approach, uirefCC,uirefCVC,displayWidget,source,outputPath).run
          println("Done!")
