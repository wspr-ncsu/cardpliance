...
import org.argus.saf.Main.allOptions
...
object Main extends App {
  ...
  private def createOptions(): Unit = {
    ...
    val uiRefCCOption: Option = Option.builder("urcc").longOpt("uiRefCC").desc("Set directory for UIRef CC ID input. [Default: .]").hasArg(true).argName("urcc").build()
    val uiRefCVCOption: Option = Option.builder("urcvc").longOpt("uiRefCVC").desc("Set directory for UIRef CVC ID input. [Default: .]").hasArg(true).argName("urcvc").build()
    val displayWidgetOption: Option = Option.builder("disp").longOpt("displayWidget").desc("Set directory for Display Widget ID input. [Default: .]").hasArg(true).argName("disp").build()
    ...
    taintOptions.addOption(uiRefCCOption)
    taintOptions.addOption(uiRefCVCOption)
    taintOptions.addOption(displayWidgetOption)
    ...
    allOptions.addOption(uiRefCCOption)
    allOptions.addOption(uiRefCVCOption)
    allOptions.addOption(displayWidgetOption)
  ...
  private def cmdTaintAnalysis(cli: CommandLine): Unit = {
    ...
    if(cli.hasOption("mo") || cli.hasOption("module")) {
      module = cli.getOptionValue("mo") match {
        ...
        case "CUSTOM_ANALYSIS" => TaintAnalysisModules.CUSTOM_ANALYSIS
      }
    }
    ...
    if(cli.hasOption("urcc")) {
      uiRefCC = cli.getOptionValue("urcc")
    }

    if(cli.hasOption("urcvc")) {
      uiRefCVC = cli.getOptionValue("urcvc")
    }

    if(cli.hasOption("disp")) {
      displayWidget = cli.getOptionValue("disp")
    }
    ...
    TaintAnalysis(module, debug, sourcePath, outputPath, forceDelete, guessPackage, approach,uiRefCC,uiRefCVC,displayWidget)

    var estimatedTime = (System.nanoTime() - startTime)/1000000000;

    println("Runtime: "+estimatedTime+" seconds")
    outputTime(estimatedTime,outputPath,sourcePath)

  }

  def outputTime(time: Long, path: String, sourcePath: String): Unit ={
    val pattern = "([^\\/]+$)".r
    val appName = pattern.findFirstIn(sourcePath).head
    val app = appName.substring(0,appName.length-4)
    val out = new FileWriter(path+"/"+app+"/runtime.txt")
    out.write("Runtime: "+ time/60+" minutes\n")
