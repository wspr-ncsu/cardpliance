...
object ComponentBasedAnalysis {
  ...
  def prepare(apks: ISet[ApkGlobal])(implicit timeout: FiniteDuration): Unit = {
    println("Timeout is "+ timeout._1+" minutes")
    apks.foreach { apk =>
            ...
            case te: TimeoutException => // Timeout happened
              {
                ...
                //write error in file
                val outputDirUri = FileUtil.appendFileName(apk.model.layout.outputSrcUri, "error")
                val outputDir = FileUtil.toFile(outputDirUri)
                if (!outputDir.exists()) {
                  outputDir.mkdirs()
                }
                val out = new FileWriter(FileUtil.toFile(FileUtil.appendFileName(outputDirUri, "error.txt")),true)
                out.write("Timeout Error in analyzing componenet "+ component+"\n")
                out.close()
              }
            case ex: Exception =>
