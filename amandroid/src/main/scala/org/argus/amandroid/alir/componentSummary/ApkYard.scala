...
class ApkYard(val reporter: Reporter) {
  ...
  def setUiRefCCPath(apkUri: String, uiRef:String): Unit= {

    val apk: ApkGlobal = getApk(apkUri).head
    apk.model.setUirefUriCC(uiRef)

  }


  def setUiRefCVCPath(apkUri: String, uiRef:String): Unit= {

    val apk: ApkGlobal = getApk(apkUri).head
    apk.model.setUirefUriCVC(uiRef)

  }

  def setDisplayWidgetPath(apkUri: String, displayWidget:String): Unit= {

    val apk: ApkGlobal = getApk(apkUri).head
    apk.model.setDisplayWidget(displayWidget)

  }



  def setOutputPath(apkUri: String ,output:String): Unit= {

    val apk: ApkGlobal = getApk(apkUri).head
    apk.model.setoutputPath(output)

  }

  def setSourcePath(apkUri: String ,source:String): Unit= {

    val apk: ApkGlobal = getApk(apkUri).head
    apk.model.setsourcePath(source)
