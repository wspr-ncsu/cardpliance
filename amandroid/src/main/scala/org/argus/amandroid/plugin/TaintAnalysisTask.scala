...
import org.argus.amandroid.alir.taintAnalysis.{CustomSourceAndSinkManager, DataLeakageAndroidSourceAndSinkManager}
import org.argus.amandroid.core.{AndroidGlobalConfig, ApkGlobal}
import org.argus.amandroid.core.decompile.{DecompileLayout, DecompileStrategy, DecompilerSettings}
import org.argus.amandroid.plugin.communication.CommunicationSourceAndSinkManager
import org.argus.amandroid.plugin.customAnalysis.{CCNumberRetentionCompliance, CVCRetentionCompliance, CardplianceAnalyzer, CardplianceUtils}
...
case class TaintAnalysisTask(module: TaintAnalysisModules.Value, fileUris: ISet[(FileResourceUri, FileResourceUri)], forceDelete: Boolean, reporter: Reporter, guessPackage: Boolean, approach: TaintAnalysisApproach.Value, uirefCC: String, uirefCVC: String, displayWidget: String,source: String,output: String) {
...
//  private final val TITLE = "TaintAnalysisTask"
  def run: Option[TaintAnalysisResult] = {
    ...
    if(fileUris.size==1) yard.setUiRefCCPath(fileUris.head._1,uirefCC)

    if(fileUris.size==1) yard.setUiRefCVCPath(fileUris.head._1,uirefCVC)

    if(fileUris.size==1) yard.setDisplayWidgetPath(fileUris.head._1,displayWidget)

    if(fileUris.size==1) yard.setSourcePath(fileUris.head._1,source)

    if(fileUris.size==1) yard.setOutputPath(fileUris.head._1,output)
    ...
    val ssm = module match {
      ...
      case CUSTOM_ANALYSIS =>
        new CustomSourceAndSinkManager(AndroidGlobalConfig.sourcesink)
    }
    approach match {
...
//        apks.head.getIDFGs.head._2.icfg.toDot(p)
//
//        val p2 = new PrintWriter("ddg.dot")
//        iddResult._2.getIddg.toDot(p2)

        //Custom Analysis code

        val analyzer : CardplianceAnalyzer = new CardplianceAnalyzer(yard,iddResult,tar,apks)
