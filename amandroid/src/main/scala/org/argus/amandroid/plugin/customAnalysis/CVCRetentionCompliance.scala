...
/**
  * @project argus-saf
  * @author samin on 4/30/19
  */
class CVCRetentionCompliance {
  ...
  def filterSourceSink(iddResult: (ISet[ApkGlobal], InterProceduralDataDependenceInfo), tar:TaintAnalysisResult, yard: ApkYard): Tar ={
    ...
    val uiRefIds: Set[String] = getIDSet(apk.model.getUirefUriCVC())

    if(uiRefIds.size ==0){

      //MOD:SAMIN run the analysis without UiRef filtering if no ID found
      println("No UiRef CVC list available")
      null
    }
    else{

      println("Running Analysis with UiRef CVC filtering")
  ...
  private def getIDSet(fileName: String): Set[String] = {
    ...
    try {
      ...
      case _:Throwable=> println("Cannot load UIREF CVC file")
    }
    ids.toSet
  }
}
