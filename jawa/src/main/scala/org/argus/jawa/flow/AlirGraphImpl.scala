...
trait AlirGraphImpl[N <: AlirNode] extends AlirGraph[N] {
  ...
  def hasNext(srcNode: N, it: BreadthFirstIterator[N,Edge]): Boolean = {

    it.hasNext

  }

  def next(srcNode: N, it: BreadthFirstIterator[N,Edge]): N = {

    it.next()

  }

  def getIterator(srcNode: N): BreadthFirstIterator[N,Edge] ={

    val it = new BreadthFirstIterator(this.graph,srcNode)
    it
  }

  def getDFSIterator(srcNode: N): DepthFirstIterator[N,Edge] ={

    val it = new DepthFirstIterator(this.graph,srcNode)
    it
  }
